...
class CRACK500CropDataset(Dataset):
    """
    ...
    def __init__(self, data_root, split_mode,
        ...
        self.logger = logging.getLogger('CRACK500 Crop Dataset')
        self.logger.debug('split_mode is set to %s', self.split_mode)
        
        self.dataset = CRACK500CropPILDataset(data_root=self.data_root, split_mode=self.split_mode)
    ...
    def _default_trans(image, annot, split_mode):
        ...
        # image = TF.resize(image, size=(640, 360))
        # annot = TF.resize(annot, size=(640, 360))
...
class CRACK500CropPILDataset(Dataset):
    """
    CRACK500 dataset of original PIL images
    """

    def __init__(self, data_root, split_mode):
        self.logger = logging.getLogger('CRACK500 Crop PIL Dataset')
        self.data_root = path.expanduser(data_root)
        self.split_mode = split_mode

        if self.split_mode == 'valid':
            self._dir = path.join(self.data_root, 'CRACK500', 'valcrop')
        elif self.split_mode == 'train':
            self._dir = path.join(self.data_root, 'CRACK500', 'traincrop')
        elif self.split_mode == 'test':
            self._dir = path.join(self.data_root, 'CRACK500', 'testcrop')
        ...
        assert len(self._image_paths) == len(
            self._annot_paths), 'CRACK500 Crop dataset corrupted'
        
        self.start = time.time()
        self.logger.debug('Found all %d samples for CRACK500 Crop dataset',
...
if __name__ == '__main__':
    ...
    CRACK500_dataset = CRACK500CropDataset('../data/', 'test',transforms=None, download=False, extract=False)
